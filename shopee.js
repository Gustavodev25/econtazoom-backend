...
// ROTA DE AUTENTICAÇÃO - MODIFICADA
router.get('/auth', (req, res) => {
    ...
    try {
        // A URL de redirect agora é "limpa", sem parâmetros
        ...
        // Passamos o UID no parâmetro 'state'
        authUrl.searchParams.append('state', uid); 
        
        console.log(`[Shopee Auth] Redirecionando para autenticação com Partner ID: ${CLIENT_ID} e State: ${uid}`);
...
// ROTA DE CALLBACK - MODIFICADA
router.get('/callback', async (req, res) => {
    // O UID agora vem do parâmetro 'state'
    const { code, shop_id, state } = req.query;
    const uid = state;

    if (!code || !shop_id || !uid) {
        return res.status(400).send('Parâmetros de callback (code, shop_id, state/uid) ausentes.');
    }
    ...
    try {
        ...
        if (tokenResponse.data.error) {
            throw new Error(`Erro ao buscar token: ${tokenResponse.data.message}`);
        }
        ...
        if (shopInfoResponse.data.error) {
            throw new Error('Falha ao obter os detalhes da loja.');
        }
        ...
        console.log(`[Shopee Callback] Sucesso para UID: ${uid} e Loja: ${shop_id}`);
        res.send('<script>window.close();</script><h1>Autenticação concluída!</h1><p>Pode fechar esta janela.</p>');
    } catch (error) {
